...
var depsMismatch = function depsMismatch(deps, lastDeps) {
  // Always
  if (deps === undefined) {
    return true;
  } // First time
...
var warnIfDepsSizeChangedOnDEV = function warnIfDepsSizeChangedOnDEV(deps, lastDeps) {
  if (true) {
    if (!(deps === undefined && lastDeps === undefined || deps !== undefined && lastDeps === undefined || deps !== undefined && lastDeps !== undefined && deps.length === lastDeps.length)) {
