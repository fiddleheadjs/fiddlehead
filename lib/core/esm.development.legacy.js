...
var compareObjects = function compareObjects(A, B) {
  ...
  for (var i = keysA.length - 1; i >= 0; i--) {
    if (!(hasOwnProperty.call(B, keysA[i]) && _is(A[keysA[i]], B[keysA[i]]))) {
      return false;
    }
  }
...
var _reconcileOnlyChildOfDynamicNode = function _reconcileOnlyChildOfDynamicNode(current, alternate, isRenderRoot) {
  if (alternate !== null) {
    ...
    current.updateId_ = alternate.updateId_; // If this node itself did not triggered the re-render,
    ...
    if (isRenderRoot) {
      ...
      if (current.alternate_ !== null) {
        if (current.alternate_ === current) {
          console.log('not walk deeper', current.type_.name); // This node does not changed,
          // stop walking deeper
...
export { Fragment, TextNode, createElement, createPortal, createRef, createElement as jsx, render, useCatch, useEffect, useLayoutEffect, useRef, useState, resolveRootVNode as useTreeId };
