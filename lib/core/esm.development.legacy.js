...
var updateNativeTextContent = function updateNativeTextContent(node, text) {
...
var _updateElementAttribute = function _updateElementAttribute(element, attrName, newAttrValue, oldAttrValue) {
  ...
  if (_canBeAttribute(attrName, newAttrValue)) {
    element.setAttribute(attrName, newAttrValue); // Continue handle as properties
  }
...
var _removeElementAttribute = function _removeElementAttribute(element, attrName, oldAttrValue) {
  ...
  if (_canBeAttribute(attrName, oldAttrValue)) {
    element.removeAttribute(attrName); // Continue handle as properties
  }
...
var hydrateView = function hydrateView(vnode) {
  ...
  if (vnode.type_ === TextNode) {
    nativeNode = createNativeTextNode(vnode.props_); // In the production mode, remove text content from the virtual text node
...
var rehydrateView = function rehydrateView(newVNode, oldVNode) {
  ...
  if (newVNode.type_ === TextNode) {
    updateNativeTextContent(newVNode.nativeNode_, newVNode.props_); // In the production mode, remove text content from the virtual text node
    // to save memory. Later, we will compare the new text with the text content
    // of the native node, though it is not a perfect way to compare.
...
var _mountEffect = function _mountEffect(hook) {
  ...
  hook.lastDestroy_ = hook.destroy_; // Run effect callback
...
var _destroyEffect = function _destroyEffect(hook, isUnmounted) {
