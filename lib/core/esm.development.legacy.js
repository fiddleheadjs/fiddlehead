...
var renderTree = function renderTree(current) {
  ...
  _workLoop(current, _performUnitOfWork, _onSkip, _onReturn, effectMountNodes, effectDestroyNodes); // Cleanup
...
var _performUnitOfWork = function _performUnitOfWork(current, root, effectMountNodes, effectDestroyNodes) {
  ...
  reconcileChildren(current, isRenderRoot); // Indicate that whether the work loop should skip the subtree under the current
...
var _workLoop = function _workLoop(root, performUnit, onSkip, onReturn, D0, D1) {
  var current = root;
  var skipFrom = null;
...
export { Fragment, Text, createElement, createPortal, createRef, createElement as jsx, render, useCallback, useCatch, useEffect, useLayoutEffect, useMemo, useRef, useState, resolveRootVNode as useTreeId };
