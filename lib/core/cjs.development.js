 ...
 */
function createPortal(content, nativeNode) {
 ...
 * @returns {VNode}
 */
function createVNodeFromPortalElement(element) {
 ...
 * @param {any[]} content
 */
 function setChildrenFromContent(current, content) {
 ...
 * @param {any} content
 */
function setChildFromContent(current, content) {
 ...
 * @returns {VNode}
 */
 function createVNodeFromJSXElement(element) {
    // Type and content
...
function _reconcileChildOfDynamicNode(current, alternate, isRenderRoot) {
    if (alternate !== null) {
        ...
        // Don't forget to clean up the alternate,
