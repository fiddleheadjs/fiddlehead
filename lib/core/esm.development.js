 ...
 * @param {VNode} vnode 
 */
const attachVNodeToNativeNode = (nativeNode, vnode) => {
 ...
 */
const extractVNodeFromNativeNode = (nativeNode) => {
 ...
 * @param {Node} nativeNode
 */
 const linkNativeNodeWithVNode = (vnode, nativeNode) => {
 ...
 */
const createPortal = (content, nativeNode) => {
 ...
 * @returns {VNode}
 */
const createVNodeFromPortalElement = (element) => {
 ...
 * @returns {VNode}
 */
const resolveMountingPoint = (current) => {
    while (true) {
        if (current === null) {
            return null;
        }
 ...
 * @returns {void}
 */
const walkNativeChildren = (callback, parent, stopBefore) => {
...
const updateNativeTextContent = (node, text) => {
...
const removeNativeNode = (nativeNode) => {
    if (nativeNode.parentNode !== null) {
        nativeNode.parentNode.removeChild(nativeNode);
    }
};

const insertNativeNodeAfter = (parent, newChild, childBefore) => {
...
const rehydrateView = (newVNode, oldVNode) => {
...
const updateView = (newVNode, oldVNode) => {
...
const prepareCurrentlyProcessing = (functionalVNode) => {
...
const resolveCurrentRefHook = (createHookFn, processFn) => {
    _throwIfCallInvalid();
...
const resolveCurrentStateHook = (createHookFn, processFn) => {
    _throwIfCallInvalid();
...
const resolveCurrentEffectHook = (createHookFn, processFn) => {
    _throwIfCallInvalid();
 ...
 */
const mountEffects = (effectTag, vnode, isNewlyMounted) => {
 ...
 */
const destroyEffects = (effectTag, vnode, isUnmounted) => {
    let hook = vnode.effectHook_;
    while (hook !== null) {
        if (hook.tag_ === effectTag) {
 ...
 * @param {any[]} content
 */
 const setChildrenFromContent = (current, content) => {
 ...
 * @param {any} content
 */
const setOnlyChildFromContent = (current, content) => {
 ...
 * @returns {VNode}
 */
 const createVNodeFromJSXElement = (element) => {
    // Type and content
 ...
 * @return {VNode|null}
 */
const createVNodeFromContent = (content) => {
...
const reconcileChildren = (current, isRenderRoot) => {
 ...
 */
 const render = (children, targetNativeNode) => {
