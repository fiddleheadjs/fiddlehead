...
function performUnitOfWork(current, direction) {
    console.log(current.name_, direction === SubtreeRoot ? 'root_node' : (
        current.alternative_ !== null
            ? chalk.blue('update_node')
            : chalk.green('create_node')
    ));
    
    reconcileChildren(current);
    
    if (current.deletions_ !== null) {
        current.deletions_.forEach(node => console.log(node.name_, chalk.red('delete_subtree')));
